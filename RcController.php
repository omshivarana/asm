...
class VehicleController extends Controller
{
    ...
    {
        $rcdetails = RcDetail::all();
        return view('backend.vehicle.vehicle', compact('rcdetails'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($user) {
            ...
            $owner_detail = [
                ...
                'rc_details_id' =>$rc_detail_inserted,
            ...
            $kyc_rc_national_permit_detail = [
                ...
                'kyc_rc_permit_id' =>$kyc_rc_permit_detail_inserted,
                'permit_number' => $request->input('permit_number'),
    ...
    public function checkVehicle(Request $request)
    {
        ...
        if ($user) {
            ...
            if ($data) 
            {
                $vehicleDetails = DB::table('rc_details')
                ->join('owner_details', 'rc_details.id', '=', 'owner_details.rc_details_id')
                ->join('financing_details', 'rc_details.id', '=', 'financing_details.rc_details_id')
                ->join('kyc_rc_permits', 'rc_details.id', '=', 'kyc_rc_permits.rc_details_id')
                ->join('kyc_rc_national_permits', 'kyc_rc_permits.id', '=', 'kyc_rc_national_permits.kyc_rc_permit_id')
                ->join('kyc_rc_vehicle_details', 'rc_details.id', '=', 'kyc_rc_vehicle_details.rc_details_id')
                ->join('kyc_rc_insurance_details', 'rc_details.id', '=', 'kyc_rc_insurance_details.rc_details_id')
                ...
                ->select(
                    ...
                    'financing_details.*',
                    'kyc_rc_permits.*',
                    'kyc_rc_national_permits.*',
                    'kyc_rc_vehicle_details.*',
                    'kyc_rc_insurance_details.*'
                )
                ->first();
                return response()->json([
                    'status' => 'success',
