...
class OrderCartController extends Controller
{
    ...
    public function orderCartDelete(Request $request)
    {
        $user_id = $this->validate_user($request->connection_id, $request->auth_code);
        
        if ($user_id) 
        {
            $orderCartDelete = OrderCart::find($request->cart_id);
            
            if($orderCartDelete){
                $orderCartDelete->delete();
                return response()->json(['status'=>'success','message' => 'Order cart deleted successfully'], 200);
            } else {
    ...
    public function updateCartQuantity(Request $request)
    {
        ...
        if ($user_id) 
        {
            ...
            if ($existingOrder) {
                ...
                return response()->json([
                    'status' => 'success',
                    'message' => 'Quantity and amount updated in cart successfully',
                    'data' => ['id'=>$existingOrder->id, 'product_id'=>$existingOrder->product_id, 'item_name'=>$existingOrder->item_name, 'quantity'=>$totalQuantity, 'unit_price'=>$existingOrder->unit_price, 'taxes'=>$existingOrder->taxes, 'total_amount'=>$totalAmount]
                ], 200);
            } else {
                return response()->json([
